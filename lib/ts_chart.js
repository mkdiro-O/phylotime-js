 ...
 * @param {string[]} opts.allSubjects - Ordered list of all subject IDs.
...
export function create_abundance_timesearcher(opts) {
  ...
  function update(nodeValues, nodeName, subjectIds) {
    d3.select(container).selectAll("*").remove();

    if (!nodeValues || !subjectIds || subjectIds.length === 0) {
      d3.select(container).append("p")
        .style("color", "#999").style("font-style", "italic")
        .text("Select a tree node and brush subjects on the PCA plot.");
      return;
    }

    const filterSet = new Set(subjectIds);
    const series = build_series(nodeValues, allSubjects, days, filterSet);
    ...
    function on_timebox_update(constraints) {
      ...
      for (const s of series) {
        if (constraints.every(c => series_passes_through(s, c, xScale))) {
          selectedSubjects.add(s.subject);
          selectedSeries.push(s);
        }
      }
      update_line_selection(lines, dots, selectedSubjects, true, dayColorScale, style);
