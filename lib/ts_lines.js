 ...
 */
export function build_series(values, mouseIds, days, filterMice) {
...
export function setup_line_hover(svg, series, lines, dots, xScale, yScale, w, h, isTimeboxActive, dayColorScale, style) {
  ...
  hoverOverlay.on("mousemove", function(event) {
    ...
    lines
      .attr("stroke", d => d.mouse === closest ? "#555" : style.lineColor)
      .attr("stroke-width", d => d.mouse === closest ? 2.5 : style.lineWidth)
      .attr("opacity", d => d.mouse === closest ? 1 : (tbActive ? 0.05 : 0.3));

    dots
      .attr("fill", d => d.mouse === closest ? dayColorScale(d.Day) : style.lineColor)
      .attr("opacity", d => d.mouse === closest ? 1 : (tbActive ? 0.05 : 0.2))
      .attr("r", d => d.mouse === closest ? style.dotRadius * 2 : style.dotRadius);
...
/**
 * Updates visual state of lines/dots based on selected mice.
 * @param {d3.ScaleOrdinal} dayColorScale
 * @param {Object} style - { lineColor, lineWidth, dotRadius }
 */
export function update_line_selection(lines, dots, selectedMice, hasActive, dayColorScale, style) {
  if (!hasActive) {
  ...
  lines
    ...
    .attr("stroke", d => selectedMice.has(d.mouse) ? "#888" : style.lineColor)
    .attr("stroke-width", d => selectedMice.has(d.mouse) ? 2 : 1);
  dots
    .attr("opacity", d => selectedMice.has(d.mouse) ? 1 : 0.05)
    .attr("fill", d => selectedMice.has(d.mouse) ? dayColorScale(d.Day) : style.lineColor)
    .attr("r", d => selectedMice.has(d.mouse) ? style.dotRadius * 1.6 : style.dotRadius * 0.8);
}
